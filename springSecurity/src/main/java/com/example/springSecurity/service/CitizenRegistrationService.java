...
public class CitizenRegistrationService {
    ...
    public Map<String, String> validate(CitizenRegisterDto dto) {
        ...
        if (isBlank(dto.getMobileNo())
                || !MOBILE_PATTERN.matcher(dto.getMobileNo().trim()).matches()) {

            errors.put("mobileNo", "Valid 10-digit mobile number is required");
        }

        if (isBlank(dto.getUsername())
                || dto.getUsername().trim().length() < 4) {
        ...
        if (!errors.containsKey("username")
                && appUserRepository.findRoleIdByRoleName(CITIZEN_ROLE).isEmpty()) {

            errors.put("detail",
                    CITIZEN_ROLE + " role is not configured.");
        }
    ...
    @Transactional
    public Users register(CitizenRegisterDto dto) {

        Integer roleId = appUserRepository
                .findRoleIdByRoleName(CITIZEN_ROLE)
                .orElseThrow(() ->
                        new IllegalStateException(
                                "Role not found : " + CITIZEN_ROLE));
