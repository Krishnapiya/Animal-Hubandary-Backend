...
@ToString
@NoArgsConstructor
public class DogBreederApplicationDocument extends AbstractEntity{
	 ...
	 private static final long serialVersionUID = 1L;
	    ...
	    public <K extends AbstractDto> void copyFromDTO(K dto) {
	        ...
	        DogBreederApplicationDocumentDto documentDto = (DogBreederApplicationDocumentDto) dto;

	        if (ValidationUtils.isValid(documentDto.getId()))
	            id = documentDto.getId();

	       if (ValidationUtils.isValid(documentDto.getApplicationId()))
	            application = new DogBreederRegistrationApplication(documentDto.getApplicationId());

	        if (ValidationUtils.isValid(documentDto.getDocumentTypeId()))
	            documentType = new DocumentType(documentDto.getDocumentTypeId());
	    ...
	    @Override
	    public DogBreederApplicationDocumentDto toDTO() {

	    	DogBreederApplicationDocumentDto dto = new DogBreederApplicationDocumentDto();
	    ...
	    public DogBreederApplicationDocument(Long id) {
	        this.id = id;
	    }
}
