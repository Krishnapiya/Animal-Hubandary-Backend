...
public class RoleMasterPredicates {
	public static Specification<RoleMaster> createPredicate(RoleMasterSearchBean searchBean) {
		return (root, query, criteriaBuilder) -> {
			List<Predicate> predicates = new ArrayList<>();
			if (ValidationUtils.isValid(searchBean.getRoleName())) {
				predicates.add(criteriaBuilder.like(criteriaBuilder.lower(root.get("roleName")),
