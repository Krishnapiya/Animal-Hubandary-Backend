...
@Getter
@Setter
@Table(name = "complaint_document", schema = "awb")
@Entity
@ToString
@NoArgsConstructor
public class ComplaintDocument extends AbstractEntity {
    ...
    public <K extends AbstractDto> void copyFromDTO(K dto) {
        ...
        ComplaintDocumentDto documentDto =
                (ComplaintDocumentDto) dto;

        if (ValidationUtils.isValid(documentDto.getId()))
            id = documentDto.getId();

        if (ValidationUtils.isValid(documentDto.getComplaintId()))
            complaint =
                    new ComplaintRegistration(
                            documentDto.getComplaintId());
        ...
        if (ValidationUtils.isValid(documentDto.getUploadedBy()))
            uploadedBy =
                    new Users(documentDto.getUploadedBy());
    ...
    @Override
    public ComplaintDocumentDto toDTO() {

        ComplaintDocumentDto dto =
                new ComplaintDocumentDto();
    ...
    public ComplaintDocument(Long id) {
        this.id = id;
    }
}